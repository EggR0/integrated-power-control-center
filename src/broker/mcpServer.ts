...
async function callTool(broker: IntegratedPowerBroker, name: string, args: Record<string, any>): Promise<unknown> {
  switch (name) {
    case "integrated_power_create_task":
      return { task: await broker.createTask(args as CreateTaskInput) };
    case "integrated_power_delegate":
      return { event: await broker.delegate(args as DelegateTaskInput) };
    case "integrated_power_debate":
      return { result: await broker.debate({ taskId: String(args.taskId), providers: Array.isArray(args.providers) ? args.providers : [], prompt: String(args.prompt ?? ""), expectedRevision: Number(args.expectedRevision), idempotencyKey: args.idempotencyKey }) };
    ...
    case "integrated_power_request_approval":
      return { approval: await broker.requestApproval(String(args.taskId), args.action, String(args.description ?? ""), args.requestedBy ?? "user", optionalRevision(args.expectedRevision), args.idempotencyKey) };
    case "integrated_power_cancel":
      await broker.cancel(String(args.taskId), Number(args.expectedRevision), args.idempotencyKey);
      return { ok: true };
    case "integrated_power_merge":
      return { commit: await broker.mergeTask(String(args.taskId), Number(args.expectedRevision), args.idempotencyKey) };
